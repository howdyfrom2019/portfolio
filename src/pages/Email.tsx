...
const Email: React.FC<PortalProps> = ({ className, style, close, onClose }) => {
  ...
  const [isDesktopEnv, setIsDesktopEnv] = useState(true);
  ...
  useEffect(() => {
    setIsDesktopEnv(checkEnv() === "desktop");
  }, []);
  ...
  return (
    <EmailPortal close={close} className={"overflow-y-scroll"}>
      ...
      <div className={"z-1000 flex justify-between min-h-80vh h-fit w-full m-[auto] max-w-contents p-11"}>
        <form className={"max-w-[555px] w-[38.54vw] h-full bg-white flex flex-col mt-2vw md-2vmax pt-8 p-7 md:max-w-full md:w-full"} ref={formRef} name={"email-sheet"}>
        ...
        {isDesktopEnv && <article className={"max-w-[555px] w-[38.54vw] h-full bg-white flex flex-col mt-2vw md-2vmax pt-8 p-7"}>
          <section className={"flex flex-col font-notoRegular"}>
            ...
            <div className={"flex flex-col mt-6"}>
              <div className={"flex gap-3 font-notoMedium items-center"}>
                ...
                <Button
                  className={"pt-[2px!important] pb-[2px] px-3 font-notoMedium rounded-full text-gray cursor-default"}>VIP</Button>
              </div>
